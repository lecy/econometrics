...
For our first example, let's assume we have data on the total consumption of tobacco in a US state from a point in time (t0) to ten years later (t10). Our observations are represented in the graph 1 below. The line suggests an upward trend, with state residents consuming more tobacco over time. 

```{r f1, fig.cap="Figure 1. Yearly consumption of tobacco from t0 to t10", out.width='60%', echo=F }
...
We can represent some possible outcomes in figures 2 - 5. 

In figure 2, the tax has no effect on tobacco consumption, which continues to increase at the same trend as before. 

```{r f2, fig.cap="Figure 2. No policy effect", out.width='60%', echo=F }
knitr::include_graphics( "FIGURE/TimeSeries/Picture2.1.png" )
```

In figure 3, the tax reduces tobacco consumption right after the policy intervention but the upward trend continues over time. There is an **immediate policy effect** but no sustained effect.

```{r f3, fig.cap="Figure 3. Immediate policy effect", out.width='60%', echo=F }
knitr::include_graphics( "FIGURE/TimeSeries/Picture2.2.png" )
```

In figure 4, the tax does not reduce tobacco consumption immediately, but it shifts the trend downward in the long term. There is no immediate policy effect but we observe a **sustained effect** of the policy.

```{r f4, fig.cap="Figure 4. Sustained policy effect", out.width='60%', echo=F }
knitr::include_graphics( "FIGURE/TimeSeries/Picture2.3.png" )
```

Finally, we can also imagine that the tax decreases tobacco consumption immediately after AND shifts the trend downward. Figure 5 shows **both an immediate and a sustained policy effect**. 

```{r f5, fig.cap="Figure 5. Immediate + sustained policy effect", out.width='60%', echo=F }
...
* has no effect (2); OR  
* has only an immediate effect (3); OR
* has only a sustained, long term effect (4); OR
* has both an immediate and a sustained effect (5). 
...
```{r fdataset, fig.cap="Figure 6. A time series dataset", out.width='60%', echo=F }
knitr::include_graphics( "FIGURE/TimeSeries/Picture5.png" )
```

We can graphically illustrated each coefficient and gradually build equation (1). 

In figure 7, $\text{b}_0$ is the baseline level of the outcome (the intercept or constant) and $\text{b}_1$ is the **slope of the line before the intervention** (green line). 
...
```{r f7, fig.cap="Figure 8. Coefficient b0, b1, and b2, if b3 = 0", out.width='60%', echo=F }
knitr::include_graphics( "FIGURE/TimeSeries/Picture3.2.png" )
```

In figure 9, the slope has changed after the intervention. $\text{b}_3$ is the **difference between the slope of the line before and the slope of the line after the intervention**. It represents the **sustained effect** of the policy intervention. Note that $\text{b}_2$ = 0,  meaning that there has been no immediate effect of the policy.
...
```{r f8, fig.cap="Figure 9. Coefficient b0, b1, and b3, with b2 = 0", out.width='60%', echo=F }
knitr::include_graphics( "FIGURE/TimeSeries/Picture3.3.png" )
```

Finally, in figure 10, we have **both an immediate and sustained effect** and $\text{b}_2$ and $\text{b}_3$ are both different from zero.
...
```{r f9, fig.cap="Figure 10. Coefficient b0, b1, b2, and b3", out.width='60%', echo=F }
...
Let's take a case where our policy intervention had both an immediate and a sustained effect, as shown in figure 11. The green line is represented by the equation (1), where $\beta{b}_2$ and $\beta{b}_3$ are both different from 0. 
...
To calculate the counterfactual, we need to assume that the intervention has never occured and there has been no immediate nor sustained effect. In other words, if the policy intervention had not happenend, the green line would have continued as shown by the dotted gray line. Our equation (1) would become:
...
```{r f10, fig.cap="Figure 11. Time series counterfactual ", out.width='60%', echo=F }
knitr::include_graphics( "FIGURE/TimeSeries/Picture4.png" )
```

We can calculate the counterfactual for each point in time. For instance, we look at t = 12. The predicted Y is calculated in the equation (7). The predicted Y for t = 12 falls on the green line in figure 12. Its counterfactual is calculcated in equation (8). It is located on the dotted line, perpendicularly above the predicted Y. 
...
```{r f11, fig.cap="Figure 12. Time series counterfactual, T=12 ", out.width='60%', echo=F }
...
```{r f12, fig.cap="Figure 13. Time series counterfactual, T=18 ", out.width='60%', echo=F }
...
A useful exercise is to calculate outcomes at different points in time as we did in section "The counterfactual". For instance, we can calculate the outcome right after the intervention, which occured at time = 200. Note that while $\text{Time}$ = 201, $\text{Time Since Treatment}$ is equal to 1 because it is the first day after the intervention.
...
```{r ft1, fig.cap="Figure 14. Wellbeing level at t = 201"}
...
```{r ft30, fig.cap="Figure 15. Wellbeing level at t = 230"}
...
Using the same mechanism illustrated in section "The counterfactual" and "Plotting the results", we can calculate the counterfactual for any point in time. The counterfactual at $\text{Time}$ = 230 is the level of wellbeing at that point in time *if the intervention had not occured*. 

Note the changes in our equation (13): $\text{Treatment}$ and $\text{Time Since Treatment}$ are both equal to 0.
...
```{r ft30c, fig.cap="Figure 16. Counterfactual for t = 230"}
...
```{r ft320, fig.cap="Figure 17. Wellbeing level and counterfactual for t = 320"}
...
Finally, we can look at the results by plotting all predicted outcomes and their counterfactuals. In figure 18 We can see that there is an increasing in the wellbeing index after the intervention. The dashed line represents the counterfactual.

```{r allValues, fig.cap="Figure 18. Time series and its counterfactual"}
...
data( county_month_birth_rate_2014_version, package="Wats" )

# We rename the data set to avoid the long name!
data <- county_month_birth_rate_2014_version 
...
Year                 |Year (1990-1999)                                      |
Month                |Month                                                 |
fecund_population     |Fecund population                                     | 
birth_count           |Number of births in the month                         | 
Date                 |Date when the data were collected                     |
days_in_month          |Number of days in the month                           |
days_in_year           |Number of days in the year                            |
stage_id              |Indicate the time 9 months after the bombing          |
birth_rate_monthly     |Birth                                                 |
...
oklahoma <- subset(data, data$county_name == "oklahoma") 
...
```{r f13, fig.cap="Figure 20. Oklahoma county, Birth rate over time (Source: Rodgers et al., 2005)", echo=F, fig.width=10 }
...
When preparing data for time series analysis, we need to create three new variables to apply our time series equation (1): 
...
```{r fRodgers, fig.cap = "Figure 21. Observed and predicted birth rates in Oklahoma county"}
# Predict Birth rate
pred_OK <- predict( reg_OK, oklahoma )

plot( oklahoma$birth_rate_monthly,
      col = "gray",
      xlim = c( 1, nrow (oklahoma) ), 
      ylim = c( (min ( oklahoma$birth_rate_monthly ) - 0.5), (max (oklahoma$birth_rate_monthly) + 0.5)),
      xlab = "Time (days)", 
      ylab = "Birth rate" )
...
```{r fres, fig.cap="Figure 22. Residuals of a time series"}

regTS_OK <- lm(birth_rate_monthly ~ time, data = oklahoma)
...
Some times, it can be difficult to detect small correlation patterns using the residual plot as in figure 22. As alternative, we can test autocorrelation of residuals using the Durbin-Watson test in the package **lmtest**.
...
```{r}
#dwtest(oklahoma$birth_rate_monthly ~ oklahoma$time)
```
...
```{r facf, fig.cap="Figure 23. Autocorrelation plt"}
regTS_OK <- lm(birth_rate_monthly ~ time, data = oklahoma)
...
In figure 23, the x axis represents the time while the y axis is the level of autocorrelation. If a line goes outside the blue horizontal line, the autocorrelation is significant at that point in time (p-value <= 0.05). 
...
Thinking about **when** the effect is likely to occur is importan in time series. As we mentioned in section "The counterfactual", we cannot test if the slope difference is significant at different point in time. 
...
```{r regTOmean, fig.cap="Figure 25. Regression to the mean", out.width='60%', echo=F }
...
```{r f16, fig.cap="Figure 26. Time series with control group", out.width='60%', echo=F }
...
```{r f17, fig.cap="Figure 27. Time series with intervention at different points in time", out.width='60%', echo=F }
