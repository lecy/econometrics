...
```{r distr, fig.cap= "Figure 1. Distribution of a normal variable"}
...
```{r image, fig.cap = "Figure 2. US Supreme Courte, Washington DC", out.width='80%', echo=F, fig.align='center' }
...
```{r residuals, fig.cap= "Figure 3. Residuals vs fitted values"}
...
```{r binaryfunction, fig.cap= "Figure 4. Binary function"}
...
```{r VarRange, fig.cap="Figure 5. Variable transformation", out.width='40%', echo=F }
...
```{r prOR, fig.cap = "Figure 6. Relationship between probability and odd ratio"}
...
2. Yet probabilities are distributes along a s-shaped line. While in linear models the relationship between X and Y is constant, in non-linear models the relationship varies according to  the value of x (look again at graph 4).
...
We can plug this number into our formula (2). 
...
```{r GPAplot, fig.cap= "Figure 7. Probability of being admit based on GPA"}
...
```{r tang, fig.cap="Figure 8. Variable transformation", out.width='85%', echo=F }
...
```{r figure1, fig.cap="Figure 9. Covariance structure of an omitted variable model", out.width='40%', echo=F }
