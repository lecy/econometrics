...
```{r f1, fig.cap="Figure 1. Relationship between economic growth and international aid", out.width='60%', echo=F }
...
```{r f2, fig.cap="Figure 2. A negative relationship between growth and international aid?", out.width='60%', echo=F }
...
The graph 3 represents observations from three countries across three time periods.

```{r f3, fig.cap="Figure 3. Relationship between growth and international aid: panel data", out.width='60%', echo=F }
...
```{r f4, fig.cap="Figure 4. Relationship between growth and international aid: **Across** countries variation", out.width='60%', echo=F }
...
```{r f5, fig.cap="Figure 5. Relationship between growth and international aid: **Within** countries variation", out.width='60%', echo=F }
...
Let's take a look at a simulated dataset that replicates the example illustrated in figure 3. We plot the observations on a graph. We have three countries (A, B, C) and we observe the relationship between international aid and economic growth at three points in time. 
...
```{r f6, fig.cap = "Figure 6. Simulated dataset: Economic growth and international aid"}
...
First, we estimate the relationship between x and y considering only one year of data (cross-sectional dataset as in figure 2). We can use an **OLS model**. 
...
```{r f7, fig.cap = "Figure 7. Simulated dataset: Year = 1"}
...
```{r, echo=T, results='asis', fig.cap="Figure 8. OLS, Cross-sectional data, Year = 1"}
...
Since we have data across multiple years, we can also use a **pooled OLS** regression, where we use all observations across years to predict Economic Growth (as in figure 3). 
...
```{r f8, fig.cap = "Figure 9. Simulated dataset: All observations"}
...
```{r, echo=T, results='asis', fig.cap="Figure 8. Pooled OLS regression"}
...
De-meaning requires you to substract the mean from each variable. In this case, we are interested in demeaning by group, so that the values of x and y are centered around each group mean. You can observe the process in table 5. 
...
```{r f9, fig.cap="Figure 10. Relationship between growth and international aid: Within countries variation", out.width='60%', echo=F }
...
As a result, we obtain a new dataset where our observations are disposed along a line. In graph 11, there are no longer significant differences **across** the 3 countries - they all lay on the same line. We observe only **within** countries differences. 
...
```{r f10, fig.cap = "Figure 11. Simulated dataset: De-meaned observations"}
...
For instance, we can plot the relationship between x and y in our dataset in graph 12. The line represent a basic regression where X predicts Y.

```{r graph1, fig.cap = "Figure 12. Relationship between X and Y"}
...
```{r graph2, fig.cap = "Figure 13. Fixed effect model"}
...
```{r labs, fig.cap="Figure 14. Source: [Wikimedia](https://commons.wikimedia.org/wiki/File:Argonne_lab_education.jpg)", out.width='60%', echo=F }
...
Each company has different levels of investment in research and development. We plot each company average R&D investement in graph 15. Each bar represents a company. The middle dot is the average R&D investment. The bars go from the minimum level of investment to the maximum.
...
```{r companies, fig.cap= "Figure 15. Heterogeneity across companies"}
...
Variation occurs also across years. We can plot each year in graph 16. Year 2014 shows great variability than year 2, for instance.
...
```{r years, fig.cap = "Figure 16. Heterogeneity across years"}
...
```{r companyYears, fig.cap="Figure 17.  Heterogeneity across companies and years", fig.height=10}
...
We can plot our observations and the OLS results on graph 18. The black dots represent our observations. The red line represents OLS results.

We can see that the line only approximates the actual observations.

```{r plotOLS, fig.cap = "Figure 18. OLS results"}
...
```{r plotdummy, fig.cap= "Figure 19. Plot of OLS with dummy variables", fig.height=10}

data$yhat <- regDummy$fitted.values
...
abline( lm( RD ~ RDPub, data = data ), lwd = 3, col = "red" )
```
...
As mentioned in section "Key notes on fixed effects", fixed effects assume that there is no correlation across entities' error term and constants - i.e., that each entity is independent from others. 
